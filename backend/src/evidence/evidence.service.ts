...
export class EvidenceService {
  ...
  list(companyId: string) {
    return this.prisma.evidence.findMany({ where: { companyId }, orderBy: { createdAt: 'desc' }, take: 100 });
  }

  async getOne(companyId: string, id: string) {
    const row = await this.prisma.evidence.findFirst({ where: { id, companyId }, include: { frames: true } });
    if (!row) throw new NotFoundException('Evidence not found');
    return row;
  }

  async getDownloadUrl(companyId: string, id: string) {
    const evidence = await this.prisma.evidence.findFirst({ where: { id, companyId } });
    if (!evidence) throw new NotFoundException('Evidence not found');
    if (evidence.status !== 'ready' || !evidence.packKey) {
      throw new BadRequestException('Evidence pack is not ready for download');
    }
    if (!this.storage.isConfigured()) throw new BadRequestException('B2 storage is not configured');
    return this.storage.presignGet(evidence.packKey, 900);
  }
  ...
  async processRecordingEvidence(companyId: string, recordingId: string, evidenceId: string, maxFramesPerSegment = 8) {
    ...
    if (!this.storage.isConfigured()) throw new BadRequestException('B2 storage is not configured');
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ldp-evidence-'));
    let globalFrame = 0;
    const manifest: any[] = [];
    try {
      ...
      for (const segment of recording.segments) {
        ...
        const videoPath = path.join(tmpDir, `segment_${segment.sequence}.webm`);
        ...
        for (const frameFile of frames) {
          const frame = fs.readFileSync(path.join(tmpDir, frameFile));
          const frameChecksum = createHash('sha256').update(frame).digest('hex');
          ...
          manifest.push({ sequence: globalFrame, segment: segment.sequence, b2Key: key, checksum: frameChecksum });
          globalFrame++;
        }
      }
      ...
      const packBuffer = Buffer.from(JSON.stringify(pack, null, 2));
      const packChecksum = createHash('sha256').update(packBuffer).digest('hex');
      const packKey = `${companyId}/evidence/${evidenceId}/pack.json`;
      ...
      this.logger.error(`Evidence processing failed: ${e?.message || e}`);
      await this.prisma.evidence.update({ where: { id: evidenceId }, data: { status: 'failed' } }).catch(() => undefined);
      await this.prisma.recording.update({ where: { id: recordingId }, data: { status: 'failed' } }).catch(() => undefined);
      throw new InternalServerErrorException(e?.message || 'Evidence processing failed');
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  }

  async processLocalVideo(companyId: string, evidenceId: string, _videoPath: string) {
    const evidence = await this.prisma.evidence.findFirst({ where: { id: evidenceId, companyId } });
    if (!evidence) throw new NotFoundException('Evidence not found');
