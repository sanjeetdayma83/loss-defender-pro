...
@Module({
  imports: [EmailModule, EvidenceModule, BullModule.forRoot({ connection: { host: process.env.REDIS_HOST || '127.0.0.1', port: Number(process.env.REDIS_PORT || 6379) } }), BullModule.registerQueue({ name: QUEUE_EMAIL }, { name: QUEUE_EVIDENCE }, { name: QUEUE_NOTIFICATION })],
