...
export class RecordingsController {
  ...
  @Post(':id/stop')
  stop(@CurrentUser() u: AuthenticatedUser, @Param('id') id: string, @Body() dto: StopDto) {
    return this.recordings.stop(u.companyId, id, dto.durationSec, dto.segmentCount);
  }

  @Post(':id/segments/presign')
  presign(@CurrentUser() u: AuthenticatedUser, @Param('id') id: string, @Body() dto: PresignDto) {
