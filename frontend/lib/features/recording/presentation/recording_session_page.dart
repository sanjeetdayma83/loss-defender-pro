...
class RecordingSessionPage extends StatefulWidget {
  ...
  @override State<RecordingSessionPage> createState() => _RecordingSessionPageState();
}
...
class _RecordingSessionPageState extends State<RecordingSessionPage> {
  ...
  @override void initState() { super.initState(); _initCamera(); unawaited(_drainQueue()); }
  @override void dispose() { _segmentTimer?.cancel(); _cam?.dispose(); super.dispose(); }

  Future<void> _initCamera() async {
    try {
      if (!kIsWeb) {
        final permission = await Permission.camera.request();
        if (!permission.isGranted) { if (mounted) setState(() => _error = 'Camera permission denied'); return; }
      }
      final cameras = await availableCameras();
      if (cameras.isEmpty) { if (mounted) setState(() => _error = 'No camera found'); return; }
      ...
      if (!mounted) return;
      setState(() { _cam = controller; _camReady = true; });
    } catch (e) { if (mounted) setState(() => _error = 'Camera: $e'); }
  }

  Future<void> _toggle() async { if (_busy) return; if (_recording) { await _stop(); } else { await _start(); } }

  Future<void> _start() async {
    setState(() { _busy = true; _error = null; _status = null; });
    try {
      String? orderId = widget.orderId, warehouseId = widget.warehouseId;
      if (orderId == null || warehouseId == null) {
        ...
        final wb = warehouses.data;
        final wl = wb is Map && wb['data'] is List ? wb['data'] as List : (wb is List ? wb : <dynamic>[]);
        if (wl.isNotEmpty) warehouseId ??= (wl.first as Map)['id']?.toString();
      }
      ...
      _recordingId = id; _startedAt = DateTime.now(); _nextSequence = 0;
      await _startCameraSegment();
      setState(() { _recording = true; _elapsed = Duration.zero; });
      _scheduleRollover(); _tick();
    } on DioException catch (e) { if (mounted) setState(() => _error = e.message ?? 'Start failed'); }
  ...
  void _scheduleRollover() { _segmentTimer?.cancel(); _segmentTimer = Timer(segmentDuration, _rollover); }
  ...
  Future<void> _rollover() async {
    ...
    try {
      ...
      await _startCameraSegment(); _scheduleRollover();
      if (file != null && _recordingId != null) unawaited(_queueAndUpload(_recordingId!, file, sequence));
    } catch (e) { if (mounted) setState(() => _error = 'Segment rollover failed: $e'); if (_recording) _scheduleRollover(); }
    finally { _rolling = false; }
  }

  void _tick() {
    if (!_recording || _startedAt == null) return;
    Future.delayed(const Duration(seconds: 1), () { if (!mounted || !_recording) return; setState(() => _elapsed = DateTime.now().difference(_startedAt!)); _tick(); });
  }
  ...
  Future<void> _drainQueue() {
    final current = _drainFuture; if (current != null) return current;
    final future = _drainQueueInternal(); _drainFuture = future; return future.whenComplete(() => _drainFuture = null);
  }
  ...
  Future<void> _drainQueueInternal() async {
    ...
    for (final segment in segments) {
      ...
      } catch (_) { if (mounted) setState(() => _status = 'Offline/upload pending — will retry automatically'); break; }
    }
    ...
    for (final finalization in finalizations) {
      ...
      } catch (_) { if (mounted) setState(() => _status = 'Evidence finalization pending — will retry automatically'); }
    }
  }

  Future<void> _stop() async {
    setState(() { _busy = true; _error = null; }); _segmentTimer?.cancel();
    try {
      final recordingId = _recordingId; if (recordingId == null) throw Exception('Recording id missing');
      XFile? file; if (_cam != null && _cam!.value.isRecordingVideo) file = await _cam!.stopVideoRecording();
      if (file != null) { final sequence = _nextSequence++; await RecordingUploadQueue.instance.enqueue(recordingId: recordingId, sequence: sequence, file: file, durationSec: 45); }
      ...
      if (stillPending || finalizationPending) { if (mounted) setState(() { _status = 'Saved locally. Upload will resume automatically when connection returns.'; _recording = false; }); return; }
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Recording uploaded. Evidence processing started.')));
      await Future.delayed(const Duration(milliseconds: 600)); if (mounted) Navigator.pop(context, true);
    } on DioException catch (e) { if (mounted) setState(() => _error = e.message ?? 'Stop failed'); }
    catch (e) { if (mounted) setState(() => _error = 'Stop failed: $e'); }
    finally { if (mounted) setState(() => _busy = false); _recording = false; }
  }

  String _fmt(Duration d) { final m = d.inMinutes.remainder(60).toString().padLeft(2, '0'); final s = d.inSeconds.remainder(60).toString().padLeft(2, '0'); final h = d.inHours; return h > 0 ? '$h:$m:$s' : '$m:$s'; }
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: Column(children: [
      Expanded(child: !_camReady ? Center(child: Text(_error ?? 'Starting camera…', style: const TextStyle(color: Colors.white70))) : Stack(fit: StackFit.expand, children: [
        CameraPreview(_cam!),
