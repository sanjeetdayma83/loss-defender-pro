...
class _WarehousesScreenState extends State<WarehousesScreen> {
  ...
  List<dynamic> get _filtered {
    ...
    return _all.where((w) {
      ...
      return '${w['name']} ${w['code']} ${w['city']} ${w['address']}'.toLowerCase().contains(q);
    }).toList();
  }

  int get _active => _all.where((w) => w is Map && w['status']?.toString() == 'active').length;
  ...
  Future<void> _addWarehouse() async {
        ...
        content: SizedBox(
          ...
          child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              TextField(controller: codeCtrl, decoration: const InputDecoration(labelText: 'Code (e.g. WH-NE-001)', border: OutlineInputBorder())),
              const SizedBox(height: 12),
    ...
    try {
      ...
      await _load();
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Warehouse created')));
    } on DioException catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.response?.data?.toString() ?? e.message ?? 'Create failed')));
    }
  }
  ...
  Future<void> _toggleStatus(Map w) async {
    ...
    final confirm = await AppDialogs.confirm(context, title: next == 'inactive' ? 'Deactivate warehouse?' : 'Activate?', message: '${w['name']} → $next');
    if (confirm != true || !mounted) return;
    try {
      await ApiClient.instance.dio.patch('/warehouses/$id', data: {'status': next});
      if (!mounted) return;
      await _load();
    } on DioException catch (e) {
      if (!mounted) return;
        ...
        Padding(
          ...
          child: Row(children: [
            const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('Warehouses', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w800)),
              SizedBox(height: 4),
              Text('Manage and monitor all your warehouses from a single place.', style: TextStyle(fontSize: 13, color: AppColors.textSecondary)),
            ])),
            IconButton(onPressed: _load, icon: const Icon(Icons.refresh)),
            const SizedBox(width: 8),
            FilledButton.icon(onPressed: _addWarehouse, icon: const Icon(Icons.add, size: 18), label: const Text('Add Warehouse'), style: FilledButton.styleFrom(backgroundColor: const Color(0xFF2563EB))),
          ]),
        ),
        ...
        Padding(
          ...
          child: LayoutBuilder(builder: (context, c) {
            ...
            return GridView.count(crossAxisCount: cross, shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), mainAxisSpacing: 12, crossAxisSpacing: 12, childAspectRatio: 2.3, children: [
              _Kpi('Total Warehouses', '${_all.length}', Icons.warehouse, const Color(0xFF3B82F6)),
              _Kpi('Active', '$_active', Icons.check_circle, const Color(0xFF22C55E)),
              _Kpi('Inactive', '$_inactive', Icons.pause_circle, const Color(0xFFF59E0B)),
              _Kpi('Packing Stations', '$stationsTotal', Icons.grid_view, const Color(0xFF8B5CF6)),
            ]);
          }),
        ),
        const SizedBox(height: 12),
  ...
  Widget _list(List<dynamic> filtered) {
    if (filtered.isEmpty) return const Center(child: Text('No warehouses — add your first warehouse'));
    return ListView.separated(
      ...
      itemBuilder: (context, i) {
        ...
        return InkWell(onTap: () => setState(() => _selected = w), borderRadius: BorderRadius.circular(12), child: Container(
          ...
          child: Row(children: [
            ...
            SizedBox(width: 70, child: Text('$stations stn', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600))),
            Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3), decoration: BoxDecoration(color: active ? const Color(0xFFDCFCE7) : const Color(0xFFFEE2E2), borderRadius: BorderRadius.circular(20)), child: Text(status, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: active ? const Color(0xFF16A34A) : const Color(0xFFDC2626)))),
          ]),
        ));
      },
    );
  ...
  Widget _detail() {
    final w = _selected;
    if (w == null) return Container(margin: const EdgeInsets.only(right: 24, bottom: 24), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: AppColors.border)), child: const Center(child: Text('Select a warehouse', style: TextStyle(color: AppColors.textSecondary))));
    final status = w['status']?.toString() ?? 'active';
    ...
    return Container(
      ...
      child: ListView(children: [
        const Text('Warehouse Details', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 15)),
        const SizedBox(height: 16),
        ...
        if (stationList.isNotEmpty) ...[
          ...
          const Text('Stations', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 12)),
          const SizedBox(height: 6),
          for (final s in stationList)
            if (s is Map) Padding(padding: const EdgeInsets.only(bottom: 4), child: Text('• ${s['name'] ?? s['code'] ?? s['id']}', style: const TextStyle(fontSize: 12))),
        ],
        const SizedBox(height: 16),
        OutlinedButton.icon(onPressed: () => _toggleStatus(w), icon: Icon(status == 'active' ? Icons.pause : Icons.play_arrow, size: 18, color: status == 'active' ? Colors.red : const Color(0xFF16A34A)), label: Text(status == 'active' ? 'Deactivate' : 'Activate', style: TextStyle(color: status == 'active' ? Colors.red : const Color(0xFF16A34A))), style: OutlinedButton.styleFrom(minimumSize: const Size.fromHeight(44))),
      ]),
    );
  }

  Widget _row(String k, String v) => Padding(padding: const EdgeInsets.only(bottom: 10), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [SizedBox(width: 80, child: Text(k, style: const TextStyle(fontSize: 12, color: AppColors.textSecondary))), Expanded(child: Text(v, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600)))]));
}
